        ...
        function(
            ...
            var manageSpecimensOverview = ParentView.extend({
                ...
                initialize: function(options) {
                    ...
                    this.barcodes.fetch();
                    this.assays.fetch();
                    Dispatcher.on('fail:prepareLibBatch', this.showNoLockedAssayError, this)
                },
                events: {
                    'click #btnAddSpecimen': '_onOpenAddSpecimen',
                    'click div#filterGroup > button': '_filterGridData',
                    'click div.tile': '_filterGridData',
                ...
                _onGridMultiSelect: function(specimens) {
                    ...
                    if (specimens.length > 0) {
                    	this.$('#specimen-grid').removeClass('span12').addClass('span9');
                    	this.$('#grid-actions-panel').show();
                        this.$('#manageSpecimenPrint').removeAttr('disabled');
                        this.$('#manageSpecimenDelete').removeAttr('disabled');
                        this.$('#manageSpecimenExport').removeAttr('disabled');
                        this.$('#prepareLibraryBatchBtn').removeAttr('disabled');
                        if(this.$('div#accordion2').find('.selectedTile').data('filter').trim() === "toBeExtracted"){
                        	this.$('#extract').removeAttr('disabled');
                        }
                    } else {
                        this.$('#manageSpecimenPrint').attr('disabled', 'disabled');
                        this.$('#manageSpecimenDelete').attr('disabled', 'disabled');
                        this.$('#manageSpecimenExport').attr('disabled', 'disabled');
                        this.$('#prepareLibraryBatchBtn').attr('disabled', 'disabled');
                        this.$('#extract').attr('disabled', 'disabled');
                        this.$('#grid-actions-panel').hide();
                        this.$('#specimen-grid').removeClass('span9').addClass('span12');
                    }
                },
                ...
                _filterGridData: function(e) {
                	...
                	$(e.currentTarget).parent().find('.selectedTile img#arrow').hide()
                	$(e.currentTarget).parent().find('.selectedTile img#flask')[0].src="/ir/resources/img/icon-samples-"+$(e.currentTarget).parent().find('.selectedTile').data('filter').trim()+".png";
                    $(e.currentTarget).parent().find('.selectedTile').removeClass('selectedTile');
                    $(e.currentTarget).addClass('selectedTile');
                    $(e.currentTarget).find('img#arrow').show();
                    $(e.currentTarget).find('img#flask')[0].src="/ir/resources/img/icon-samples-"+$(e.currentTarget).parent().find('.selectedTile').data('filter').trim()+"-selected.png";
                    this.gridView._filters["show"] = $(e.currentTarget).data('filter').trim();
                ...
                showNoLockedAssayError: function() {
                	new BannerView({
                        container: this.$('.container-fluid').first(),
                        style: 'error',
                        titleKey: 'libraryBatch.assay.error',
                        sticky: true
                    }).render();
                }
            });
